...
pub fn cli_install(agent: &str, dry_run: bool) {
    match agent_install::run_install(agent_registry::REGISTRY, agent, None, dry_run) {
        ...
        Outcome::Err(msg) => crate::ui::error(&format!("error: {msg}")),
    }
}
...
pub fn cli_launch(agent: &str, model: Option<&str>, mode: Option<&str>, args: &[String]) {
    match agent_launch::run_launch(agent_registry::REGISTRY, agent, model, mode, args) {
        ...
        LaunchOutcome::NotFound(msg) => crate::ui::error(&format!("error: {msg}")),
        LaunchOutcome::UnknownAgent(msg) => crate::ui::error(&format!("error: unknown agent: {msg}")),
        LaunchOutcome::Extension(msg) => crate::ui::error(&format!("error: {msg}")),
    }
}
...
pub fn cli_run(
    ...
    let env = match crate::dev_vars::load(&cwd, stage) {
        ...
        None => {
            if let Some(s) = stage {
                crate::ui::warning("agentflare run: no .dev.vars.{s} or .dev.vars found");
            }
            Vec::new()
        }
    };
...
pub fn cli_run_headless(agent: &str, prompt: &str, timeout: std::time::Duration) -> i32 {
    match agent_launch::run_headless(agent_registry::REGISTRY, agent, prompt, timeout) {
        ...
        | agent_launch::HeadlessOutcome::Failed(msg) => {
            crate::ui::error(&format!("error: {msg}"));
            1
        }
    }
}
