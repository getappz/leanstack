    ...
    loop {
        ...
        match categorize_exit(code, &stderr) {
            ...
            ExitKind::RateLimited => {
                if remaining == 0 {
                    crate::ui::error("error: all retries exhausted");
                    std::process::exit(1);
                }
                remaining -= 1;
                if !json {
                ...
                if !json {
                    crate::ui::info(&format!("retrying with new profile ({remaining} retries left)..."));
                }
...
fn spawn_and_capture(agent: &str, args: &[String]) -> (i32, String) {
        ...
        .spawn()
    {
        Ok(child) => child,
        Err(e) => {
            crate::ui::error(&format!("error: cannot run {binary}: {e}"));
            std::process::exit(1);
        }
    };
