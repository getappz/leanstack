//! Integration test for `agentflare optimize output compress`, run against
//! the real compiled binary with a stubbed `claude` CLI on PATH — proves the
...
fn optimize_output_compress_generic_uses_the_stubbed_claude_cli() {
    ...
    // dirs::cache_dir()/agentflare/caveman/backups/ (that on-disk directory
    // name is a separate, out-of-scope stray reference in the flare-output
    // crate, not this test). Not asserted here (that path isn't worth
    // pinning down in this test); the source-content assertion above
    // already proves the compression itself took effect.
}
